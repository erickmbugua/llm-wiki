...
class TestInitVault:
    ...
    def test_creates_wiki_subdirectories(self, tmp_path: Path) -> None:
        init_vault(tmp_path / "v", "V")
        for subdir in WIKI_CATEGORIES:
            assert (tmp_path / "v" / "wiki" / subdir).is_dir()

    def test_creates_index_log_schema(self, tmp_path: Path) -> None:
        init_vault(tmp_path / "v", "V")
    ...
    def test_creates_internal_dir_and_gitignore(self, tmp_path: Path) -> None:
        init_vault(tmp_path / "v", "V")
        internal = tmp_path / "v" / ".llm-wiki"
        assert internal.is_dir()
        assert (internal / ".gitignore").read_text() == "wiki.db\n"

    def test_writes_vault_config(self, tmp_path: Path) -> None:
    ...
    def test_schema_contains_vault_name(self, tmp_path: Path) -> None:
    ...
    def test_index_has_yaml_frontmatter(self, tmp_path: Path) -> None:
        init_vault(tmp_path / "v", "V")
...
class TestVaultStats:
    def test_empty_vault_returns_zeros(self, tmp_vault: Path) -> None:
    ...
    def test_counts_pages_in_categories(self, tmp_vault: Path) -> None:
        wiki = tmp_vault / "wiki"
    ...
    def test_counts_raw_queued_files(self, tmp_vault: Path) -> None:
        (tmp_vault / "raw" / "doc.txt").write_text("content")
        (tmp_vault / "raw" / "doc2.pdf").write_text("content")
        stats = vault_stats(tmp_vault)
        assert stats["raw_queued"] == 2

    def test_total_pages_includes_all_md_files(self, tmp_vault: Path) -> None:
        wiki = tmp_vault / "wiki"
...
class TestRebuildIndex:
    def _pages(self, overrides: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    def test_rebuild_index_empty_vault(self, tmp_vault: Path) -> None:
        rebuild_index(tmp_vault)
        index = (tmp_vault / "wiki" / "index.md").read_text()
        assert "No pages yet" in index

    def test_rebuild_index_has_yaml_frontmatter(self, tmp_vault: Path) -> None:
        rebuild_index(tmp_vault)
        index = (tmp_vault / "wiki" / "index.md").read_text()
        assert index.startswith("---")
        assert "title: Index" in index

    def test_rebuild_index_groups_by_category(self, tmp_vault: Path) -> None:
        pages = self._pages(
            [
                {
    ...
    def test_rebuild_index_sorts_by_title(self, tmp_vault: Path) -> None:
        pages = self._pages(
            [
                {
    ...
    def test_rebuild_index_truncates_summary(self, tmp_vault: Path) -> None:
        long_summary = "x" * 200
        pages = self._pages(
            [
                {
    ...
    def test_rebuild_index_escapes_pipe_in_summary(self, tmp_vault: Path) -> None:
        pages = self._pages(
            [
                {
                    "file_path": "Concepts/Foo.md",
        ...
        table_line = next(line for line in index.splitlines() if "[[Foo]]" in line)
        assert "A | B" not in table_line

    def test_rebuild_index_skips_root_category(self, tmp_vault: Path) -> None:
        pages = self._pages(
            [
